...
export class FashionService {
  ...
  addFashion(fashion: Fashion): Observable<Fashion> {
    ...
    return this.http.post<Fashion>(this.apiUrl, fashion, { headers }).pipe(catchError(this.handleError));
  }

  updateFashion(id: string, fashion: Fashion): Observable<Fashion> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    return this.http.put<Fashion>(`${this.apiUrl}/${id}`, fashion, { headers }).pipe(catchError(this.handleError));
  }
